...
7. **离散化** : 当区间问题只关心元素之间的大小关系而不关心元素值时，将输入序列离散化，有助于提高求解效率或帮助解决一些特定问题。离散化实现包括 **松离散** 和 **紧离散** 。
...
- 以 $tree$ 的下标与输入数组 $nums$ 都从 0 开始为前提，修改了三种 BIT 类的实现代码。一并修改了大部分配图。
...
**那么树状数组是这一想法的实现吗？答案是：不完全是。** 树状数组确实将 $nums$ 划分成了多个区间，但并不是对任意区间 $[l,r]$ 划分连续子区间，而是通过 **「前缀区间和」作差** 的方式来得到指定区间的「区间和」，而前缀区间才是通过若干个连续子区间组成的。
...
很抽象，尤其是最后一句，现在无法得知如何处理下标的二进制数来划分输入区间，不过没关系，我们马上对大小为 8 (下标范围为 $[0,7]$)的 $nums$ 实践上述描述。 首先明确为了实现「更快地求区间和」的需求：

1. 将指定长度的 $nums$ 划分为若干区间。对 $nums$ 下标的划分动作应当是一种 **可循环的操作** 。
2. $nums$ 上的任意区间 $[l, r]$ 的区间和可由 $[0,r]$ 区间和与 $[0,l-1]$ 区间和作差得到，这是「前缀和」思想。这要求 $[0, l-1], [0,r]$  均由 $nums$ 划分结果中的 **若干相邻区间** 所构成。从这里可以看出， 对 $nums$ 的划分，不只是对 $[0,n-1]$ 的划分，而是对 $[0,k](k∈[0,n-1])$ 的划分，划分后，一定能由若干连续区间构成任意的 $[0,k](k∈[0,n-1])$ 区间 (这句话是理解树状数组的关键，请读者在阅读后文时反复体会)。
3. 子区间的区间和被 **实时维护** 。需要用一个 $tree[]$ 数组保存所有子区间和 (我们马上会知道为什么命名为 $tree$)。
4. 求给定区间的区间和，即求两次前缀区间和再作差。求前缀区间和需要通过 **某种规则** 一边寻找其子区间 $i$ ，一边将 $tree[i]$ (子区间 $i$ 的区间和) 累计到结果中。
5. 更新 $nums[i]$ 时，通过 **某种规则** 更新包含该值的所有子区间的区间和。



「灵感来源」描述中提到了「二进制表示」，以划分区间 $[0,14]$ 为例，我们先写出 14 的二进制表示 $14=(1110)_2$ 。我们要求划分动作是可循环的操作，且对于任意长度的 $nums$，都能通过同样的方式完成划分。划分区间实际上就是要确定区间的左右界。很直接地，区间 $[0,14]$ 的 **最右子区间的右界** 为 14 ， **最左子区间的左界** 是 0 。我们将最右子区间作为当前区间，从当前子区间右界下标 14 开始考虑。

一个容易想到的方法如下：

> 【区间划分方法】：从最右子区间右界 $k$ 开始，将 $k$ 的最低位 1 换成 0 ，新的 $k$ 作为当前区间的左界下标，减 1 即为其 **左邻子区间的右界下标** 。重复该操作直到当前区间右界下标为 0 (二进制数所有位都没有 1) 。

于是区间 $[0,14]$ 被划分为这三个区间 $[(0000)_2,(1000)_2)$ , $[(1000)_2,(1100)_2)$ , $[(1100)_2,(1110)_2]$ ，即 $[0,7]$ , $[8,11]$ , $[12,14]$。我们一方面保证了划分动作是循环的，同时也保证了划分后的子区间是连续的。但还存在一个小问题，当划分到最后一个子区间 $[(0000)_2,(1000)_2)$ 时，左界为 0 ，应当要退出划分区间的循环，但此时需要处理最后一个区间 ($[0,7]$)，不能直接退出，于是我们利用下一次左界「也是」0 来作为退出划分区间循环的判断条件。但要记录连续两次的界，这似乎有点麻烦，实际上我们只需做一个小调整，即可更优雅地结束划分。
...
1. 规则是固定的，因此操作是可循环的，通过这种方式，我们一定能够将 $nums$ 的任意 $[0, i]$ 区间划分为一些子区间。 也容易看出，$[0,k]$ 无论怎么划分， **一定有且只有一个以 $k$ 为右界的子区间** 。$k$ 的取值有 $n$ 种 ($0 \sim n-1$)，因此 **长度为 $n$ 的 $nums$ 划分出 $n$ 个子区间** ，这些子区间的右界是 $\{0,1,2,...,n-1\}$  ，左界通过 $(i+1)-lowbit(i+1)$ 求出，即 $\{1-lowbit(1),2-lowbit(2),3-lowbit(3),...,n-lowbit(n)\}$ ($lowbit(i)$ 就是最低位 1 所代表的数字，后续介绍其实现)。
...
4. 「将当前子区间右界下标最低位的 1 换成 0」的划分方式即为该规则 ($lowbit()$)。
5. 同 3，后续说明。
...
正数 $i$ ( $i=k+1$ 大于等于1，必为正) 的相反数 $-i$ 必是负数，[我们知道](https://en.wikipedia.org/wiki/Signed_number_representations)，负数在计算机中以补码 ([two's complement](https://en.wikipedia.org/wiki/Two%27s_complement)) 表示，**负数 $-i$ 的补码为对应正数 $i$ 的除符号位外按位取反后再加1**，即 $-i=\sim i+1$ ，恰好与上述式子与运算 $\&$ 的右边相同，于是我们给出如下 $lowbit(i)$方法，如其方法名所表达的那样， **该方法返回下标 $i$ 的二进制表示中的最低位的 1 所代表的数** 。后续我们可以用该方法方便地对当前子区间右界 $i$ 求其左界，即其左邻子区间右界 $i-lowbit(i)$ 。
...
下图展示了大小为 16 的 $nums$ (图中的 $a$ 数组) 的子区间划分 ($t[]$ 为 $tree[]$ )，16个矩形代表划分出的16个子区间。如图，$tree[0]$ 表示右界为 $nums[0]$ 的区间和 (该区间只有一个元素)，$tree[1]$ 表示右界为 $nums[1]$ 的区间的区间和 (该区间有 $nums[0]$ 和 $nums[1]$ 两个元素)，依次类推。蓝线表示 **区间包含关系** 。单点修改操作需要更新所有包含修改点的区间的区间和，寻找包含修改点的区间的过程就是 **沿着蓝线向上** 的过程。
...
更新 $nums[k]$ ，需要相应地更新包含它的所有区间的区间和 $tree[]$ 。通过前面的树状图，我们不难看到，**第一个要更新的区间和一定是 $tree[k]$** 。沿着蓝线上升，考虑蓝线连接的父子结点的下标二进制表示，我们发现 $i = i + lowbit(i)$  ($i=k+1$) 总是包含修改点 (子结点) 的下一个更大的区间 (父结点) 的右界加 1 ，每上升一层，就会找到包含修改点的更大范围的区间。$i$ 的最大值是区间右界下标 $n-1$ 加 1，即 $nums$ 的大小 $n$ 。

单点修改方法的方法签名为 `public void update(int k, int x)` ，表示更新 `nums[k] = x`  ，但对于蓝色链条上的「区间和」 $tree[i]$ 的更新，要做的是使 $tree[i]$ 加上增量，因此该 $update$ 方法调用 `private void add(int k, int x)` 方法，表示为第 $k$ 个区间和结点的区间和加上增量 $x$ 。每次更新 $tree[i]$，都通过调用 $add$ 方法从区间 $tree[i]$ 沿着蓝色链条完成所有区间和的更新，可见 $add$ 方法的主体是一个循环，蓝色链条上的区间和结点下标 $i$ 的更新我们已经知道，于是不难写出如下 $update$ 和 $add$ 方法。寥寥数行，配合 $lowbit$ 向上更新，十分奇妙。
...
给定 $nums$ 上的区间 $[l,r]$ ，求区间和。利用前缀和的思想，我们定义方法 `private int preSum(int k)` ，表示求 $nums[0]$ 到 $nums[k]$ 的和 (前 $k+1$ 项和) ，那么求 $[l,r]$ 的区间和即为 $preSum(r) - preSum(l - 1)$ 。以求 $preSUm(r)$ 为例，在「区间划分」中我们已经知道通过 `i = i - lowbit(i)` 的方式从 $i=r+1$ 开始依次求出组成 $[0,r]$ 区间的子区间的右界下标 ($i-1$)，也即区间和逻辑结点 $tree$ 的下标，依次将得到的 $tree[i-1]$ 累计即可得到要求的前缀区间和。可见 `preSum(k)` 方法的主体是一循环，循环条件是 `i > 0` ，循环终止时求出 $nums[0]$ 到 $nums[k]$ 的和，不难写出如下 $sum$ 和 $preSum$ 方法。
...
1. 单点修改时间复杂度: $O(logn)$ 。
   ...
   取决于更新结点到根结点的路径上的结点数，更新 $nums[0]$ 时路径上结点数最多，其数量为 $tree[i-1]$ 的下标 $i$ 从 $(000...001)_2$ 通过 $i += lowbit(i)$ 逐位更新到 $(n)_2$ 的更新次数 (初始算一次)。该数量不会大雨 $(n)_2$ 的位数，也即 $logn$ ，因此单点修改的时间复杂度为 $O(logn)$。

2. 区间查询时间复杂度: $O(logn)$ 。

   区间为 $[l, r]$。 取决于 $[0,r]$ 区间的连续子区间个数。根据子区间界的计算方法，子区间个数与 $r$ 的二进制数中 1 的数量有关，假设 $r$ 的二进制数有 $k$ 位，则 $n=2^k-1$ 时 1 的位数最多，共 $k$ 个， $k=log(r+1)$。根据 $sum$ 方法，先求 $[0,l-1], [0,r]$ 的前缀区间和，$l$ 不大于 $r$，因此求两次前缀区间和的时间复杂度为 $O(2*logr)$ ，$r$ 最大为 $n$ ，故区间查询的时间复杂度为 $O(logn)$。
...
有点拗口？没关系，我们先从著名的 **「逆序对」** 问题开始。求输入数组 $nums$ 的逆序对， **是树状数组的经典应用，更是一个「妙用」** 。我们指出，它正是本节标题「指定区间指定取值范围的元素数」的一个应用。以 [剑指 Offer 51. 数组中的逆序对](https://leetcode.cn/problems/shu-zu-zhong-de-ni-xu-dui-lcof/) 为例，树状数组解法代码见 [题解](https://leetcode.cn/problems/shu-zu-zhong-de-ni-xu-dui-lcof/solution/by-yukiyama-elnl/)，可以看到，其中的 BIT 类就是我们在前面提供的 PURQ BIT 类。但我们注意到，在主方法 ($reversePairs$) 中，调用 $add$ 时， **传入的增量 $x$ 是固定的 1** 。我们知道对于 PURQ BIT， $nums[k]$ 增加 $x$ 后，调用 $add(k,x)$ ，从第一个包含 $nums[k]$ 的区间的区间和开始，都增加 $x$ ，即从 $tree[k]+=x$ 开始，沿着父链使所有包含 $nums[k]$ 的区间和都加上 $x$。

那么 $add(nums[i]-1, 1)$ 是什么意思呢？在继续这个问题之前我们不妨先看下最朴素的做法是如何求逆序数的。

- 外层循环遍历 $nums$ ，对于当前元素 $nums[i]$ ，内层循环向后遍历 $nums[j](j>i)$ ，如果 $nums[i]>nums[j]$ ，就让 $nums[j]$ 记录「我前面多了一个大于我的数」，即 $countGreater[j]++$ 。
- 当程序结束时，每一个 $countGreater[i]$ 就是 $nums[i]$ 的逆序数，累计所有的 $countGreater[i]$ 就是所要求的 $nums$ 的逆序数总数。由于 $countGreater[i]$ 是在结束 $nums[i-1]$ 内层循环时确定的，因此可以在开始 $nums[i]$ 内层循环前累计。
...
现在稍微逆向思考一下，如果我们在遍历到 $nums[i]$ 时能够知道其左侧 **「小于等于」** 它的元素的数量 $coutLessEqual[i]$ ，那么用 $i$ ($nums[i]$ 的左侧的数共有 $i$ 个) 减去  $coutLessEqual[i]$  ，就相当于 $i-countLessEqual[i]=countGreater[i]$ 。遍历 $nums[i]$ 的过程中累计 $i-countLessEqual[i]$ ，遍历结束即完成求解。只需稍微调整第一版代码即可，如下。
...
class Solution {
    public int reversePairs(int[] nums) {
        int n = nums.length, ans = 0;
        int[] countLessEqual = new int[n];
        for(int i = 0; i < n; i++){
            ans += i - countLessEqual[i];
            for(int j = i + 1; j < n; j++){
                if(nums[i] <= nums[j]) countLessEqual[j]++;
            }
        }
        return ans;
    }
...
请读者先记住第二版朴素做法，我们再次回到树状数组的做法。



在执行 $add(nums[i]-1,1)$ 之前，原大小为 $n$ 的输入数组 $nums$ 已由 **「离散化」** 操作变为了这样一个数组:

> 离散化后的 $nums$ 的所有元素的取值范围变为 $[1,n]$ (松离散)，且任意一对 $nums[i]$ 与 $nums[j]$ 的大小关系与原 $nums$ 中 $nums[i]$ 与 $nums[j]$ 的大小关系一致。

例如 $\{-2,6,-10\}$ 离散化后为 $\{2,3,1\}$  。

接着分析 $add(nums[i]-1, 1)$ 的含义。这个操作是在遍历 $nums$ 过程中进行的，该操作使得 $tree$ 从 $tree[nums[i]-1]$ 开始，沿着父链上升，包含下标为 $nums[i]-1$ 的更大区间的区间和加 1 。$tree$ 的下标范围 $[0,n-1]$ 与 (松离散化后的)  $nums$ 元素值的大小范围 $[1,n]$  **一一对应**  ，即 $add$ 方法 (以及 $preSum$ 方法) 中的 $nums[i]-1$ 与原数组的 $nums[i]$ 是对应的 (这一点很关键)。 $add$ 的这一操作，相当于遍历到 $nums[i]$  时，使得对于 $nums$ 中所有大于等于 $nums[i]$ 的数 $nums[j]$，有 $countLessEqual[j]++$ 。注意，树状数组做法中不存在 $countLessEqual$ ，对应的值是通过 $preSum$ 求得的，后述。

结合下图，我们实际操作一下。按照上述方式对松离散化后的 $nums = \{3,1,5,4,7,8,6,2\}$ 中的元素依次执行 $add(nums[i]-1,1)$ 。
...
执行 ① 时，从 $tree[2]$ (包括 $tree[2]$) 开始向上，在 $tree[3]$ 和 $tree[7]$ 上都标记 `①` ，即遍历 $nums[0]=3$ 后，$tree[2]$ , $tree[3]$ , $tree[7]$ 的值加 1 。如我们所说，只要是大于等于 $nums[0]$ 的 $nums[j]$ ，此时其 $countLessEqual[j]$ 均为 1。依据该图，非常容易看出，询问任意一个 $nums[j]$ 的 $countLessEqual[j]$ ，实际上就是在求 $preSum(nums[j]-1)$ 。



我们惊讶地发现，$add$ 方法本质上完成了第二版朴素做法的内层循环所做的事，并且是以 $O(logn)$ 时间复杂度完成的。实际上完成得更多，因为朴素做法更新的是 $nums[i]$ 右侧的大于等于 $nums[i]$ 的 $nums[j] (j>i)$ 的 $countLessEqual[j]$ ，而树状数组 $add$ 方法是对所有大于等于 $nums[i]$ 的 $nums[j]$ 更新其 $countLessEqual[j]$ ，并不限制在 $nums[i]$「右侧」。比如当我们继续执行 ② $add(nums[1]-1, 1)$ (即 $add(0,1)$) 时，$nums[0]=3$ 的 $countLessEqual[0]$ ，将会通过 $add(0,1)$ 得到更新 (沿着父链方向更新 $tree[0]→tree[1]$) ，当我们要查询时，执行 $preSum(nums[0]-1)$ (即 $preSum(2)$) 可求出。

只不过我们不必也不会向前询问，在遍历 $nums[i]$ 时，我们先执行 $preSum(nums[i]-1)$ ，即可得到 $nums[i]$ 左侧 **「小于等于」** $nums[i]$ 的元素个数。这是因为左侧的数我们已经遍历过了，在遍历左侧的数时，如果它小于等于在它后面的 $nums[i]$ ，那么 $nums[i]$ 所对应的 $countLessEuqal[i]$ 已经通过 $add$ 方法加 1 了。再次强调，树状数组解法无 $countLessEual[i]$ ，其值加 1 实际上是某个 $tree[x]$ 加 1。

总之，我们将一个 $nums$ 离散化后，遍历它，先执行 $preSum(nums[i]-1)$ ，这个操作即为询问 $nums[i]$ 左侧小于等于 $nums[i]$ 的元素个数。再执行 $add(nums[i]-1, 1)$ 以使下一次查询有效。这就是本小节标题所谓的「指定区间内指定取值范围的元素数」 的含义，区间指 $[0,i-1]$ 区间，给定取值范围指 $[1, nums[i]]$ 。



我们给出执行 ① ~ ⑧ 后得到的树形图，供读者仔细验证。

![image.png](https://pic.leetcode-cn.com/1665830781-SDAcPL-image.png)



现在分析「逆序对」问题就简单多了。由于离散化不改变任意一对元素的大小关系，因此 **对原数组求逆序数等同于对离散化后的数组求逆序数** 。我们要做的是询问 $nums[i]$ 的前面，有多少个大于它的数，刚好相当于在 $nums$ 中插入 $nums[i]$ 之前， 求 $i - preSum(nums[i]-1)$ 。即 $[0,i-1]$ 范围内，有 $i$ 个数，其中「小于等于」$nums[i]$ 的有 $preSum(nums[i]-1)$ 个，因此  $i - preSum(nums[i]-1)$ 即为 $nums[i]$ 的逆序数。遍历 $nums$ 累计每一个 $nums[i]$ 的逆序数，即完成求解。



在完全理解了「遍历离散化后的 $nums$ ，在遍历过程中查询 $preSum(nums[i]-1)$ ，查询后通过 $add(nums[i]-1,1)$ 插入 $nums[i]$」 这一操作的意义后，类似的题目如下:
